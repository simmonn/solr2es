...
from solr2es import DEFAULT_ES_DOC_TYPE, Solr2EsAsync
...
class TestMigrationAsync(asynctest.TestCase):
    ...
    async def tearDown(self):
        async with aiohttp.ClientSession() as session:
            await session.post('http://solr:8983/solr/my_core/update/?commit=true', data='<delete><query>*:*</query></delete>', headers={'Content-type': 'text/xml; charset=utf-8'})
    ...
    async def test_migrate_twelve_docs_async(self):
        async with aiohttp.ClientSession() as session:
            data = ['<doc><field name="id">id_%d</field><field name="title">A %d document</field></doc>' % (i, i)
                    for i in range(0, 12)]
            await session.post('http://solr:8983/solr/my_core/update/?commit=true', data='<add>%s</add>' % ''.join(data),
                               headers={'Content-type': 'text/xml; charset=utf-8'})
            solr2es_async = Solr2EsAsync(session, self.aes, True)
